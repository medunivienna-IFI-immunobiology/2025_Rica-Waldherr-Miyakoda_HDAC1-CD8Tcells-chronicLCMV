...
setwd("./CutAndRun")
mysamples <- read.csv("CnR_DiffBind_samplesheet_H3K27ac.csv")
...
write.table(report.df, "H3K27ac_DiffBind_DESeq2_report_summitsT-th.tab", sep="\t", quote=F, row.names=F)
report.df.homer <- report.df[,c(1:4)]
report.df.homer$Strand <- "+"
write.table(report.df.homer, "H3K27ac_DiffBind_DESeq2_report_summitsT-th_forHOMER.tab", sep="\t", quote=F, row.names=F)
